package com.example.taller3movil.services
...
import com.example.taller3movil.MyLocation
import com.example.taller3movil.R
...
class MapRenderingServices(private val context: Context, private val map : MapView) {
    ...
    fun addMarker(geo: GeoPoint, title: String?=null, typeMarker: Char, descr: String){
        val marker= Marker(map)
        when (typeMarker) {
            'A' -> {
                marker.title="Ubicacion Actual"
                val icon= ResourcesCompat.getDrawable(context.resources,
                    R.drawable.baseline_location_blue_on_24, context.theme)
                marker.icon=icon
            ...
            'B' -> {
                marker.title=descr
                val icon= ResourcesCompat.getDrawable(context.resources,
                    R.drawable.baseline_location_green_on_24, context.theme)
                marker.icon=icon
                if(currentPositionMarker!=null){
                    //map.overlays.remove(currentPositionMarker)
                }
            ...
            'L' -> {
                ...
                val icon= ResourcesCompat.getDrawable(context.resources,
                    R.drawable.baseline_location_red_on_24, context.theme)
                marker.icon=icon
            ...
            'T'->{
                ...
                val icon= ResourcesCompat.getDrawable(context.resources,
                    R.drawable.baseline_location_black_on_24,context.theme)
                marker.icon=icon
                markers.add(marker)
            }
        }
    ...
    fun drawRoute(routePoints: ArrayList<GeoPoint>) {
        ...
        for (i in routePoints) {
            addMarker(i, typeMarker = 'T', descr = "")
        }
        displayRoad(road)
    }
